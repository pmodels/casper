...
static void breakpoint_fnc1(void)
{
}

#define CHECK_WIN_ERR_FUNC(err_class, fnc_stmt)  do {          \
...
static void check_rma_sync(void)
{
    ...
    if (rank == origin_rank) {
        ...
        CHECK_WIN_ERR_FUNC(MPI_ERR_LOCKTYPE, (MPI_Win_lock(-1, target_rank, assert, win)));
#endif
        debug_printf("checking MPI_ERR_RANK in RMA SYNC...\n");
        CHECK_WIN_ERR_FUNC(MPI_ERR_RANK, (MPI_Win_lock(lock_type, -2, assert, win)));
        ...
        CHECK_WIN_ERR_FUNC(MPI_ERR_RMA_SYNC, (MPI_Win_lock_all(assert, win)));
        MPI_Win_unlock_all(win);        /* close epoch */

        MPI_Win_lock_all(assert, win);  /* open epoch */
        CHECK_WIN_ERR_FUNC(MPI_ERR_RMA_SYNC, (MPI_Win_lock(lock_type, target_rank, assert, win)));
        MPI_Win_unlock_all(win);        /* close epoch */

        MPI_Win_lock_all(assert, win);  /* open epoch */
        CHECK_WIN_ERR_FUNC(MPI_ERR_RMA_SYNC, (MPI_Win_start(pscw_group, assert, win)));
        MPI_Win_unlock_all(win);        /* close epoch */

        MPI_Win_lock_all(assert, win);  /* open epoch */
        CHECK_WIN_ERR_FUNC(MPI_ERR_RMA_SYNC, (MPI_Win_fence(assert, win)));
        MPI_Win_unlock_all(win);        /* close epoch */

        /* -- no opening access epochs */
        debug_printf("checking MPI_ERR_RMA_SYNC in RMA SYNC (no opening access epoch)...\n");
        CHECK_WIN_ERR_FUNC(MPI_ERR_RMA_SYNC, (MPI_Win_complete(win)));
        CHECK_WIN_ERR_FUNC(MPI_ERR_RMA_SYNC, (MPI_Win_unlock(target_rank, win)));
        CHECK_WIN_ERR_FUNC(MPI_ERR_RMA_SYNC, (MPI_Win_unlock_all(win)));
        CHECK_WIN_ERR_FUNC(MPI_ERR_RMA_SYNC, (MPI_Win_flush(target_rank, win)));
        CHECK_WIN_ERR_FUNC(MPI_ERR_RMA_SYNC, (MPI_Win_flush_local(target_rank, win)));
        CHECK_WIN_ERR_FUNC(MPI_ERR_RMA_SYNC, (MPI_Win_flush_all(win)));
        CHECK_WIN_ERR_FUNC(MPI_ERR_RMA_SYNC, (MPI_Win_flush_local_all(win)));

        /* -- no opening exposure epochs */
        debug_printf("checking MPI_ERR_RMA_SYNC in RMA SYNC (no opening exposure epoch)...\n");
        CHECK_WIN_ERR_FUNC(MPI_ERR_RMA_SYNC, (MPI_Win_wait(win)));
    }
...
static void check_rma_op(void)
{
    ...
    if (rank == origin_rank) {
        ...
        CHECK_WIN_ERR_FUNC(MPI_ERR_RANK,
        ...
        CHECK_WIN_ERR_FUNC(MPI_ERR_COUNT,
        ...
        CHECK_WIN_ERR_FUNC(MPI_ERR_DISP,
