...
int main(int argc, char *argv[])
{
    ...
    MPI_Win_allocate_shared((MAX_MSG_SIZE + align_size) * 2, 1, MPI_INFO_NULL, MPI_COMM_WORLD,
                            &s_buf, &win);
    ...
    if (window_varied) {
        ...
        for (i = 0; i < log_val; i++) {
            bandwidth_results[i] = (double *) malloc(sizeof(double) * WINDOW_SIZES_COUNT);
        }
            ...
            if (rank == 0) {
                ...
                if (print_rate) {
                    fprintf(stdout, "%-*d%*.*f%*.*f\n", 10, curr_size,
                            FIELD_WIDTH, FLOAT_PRECISION, bw, FIELD_WIDTH, FLOAT_PRECISION, rate);
                }

                else {
                    fprintf(stdout, "%-*d%*.*f\n", 10, curr_size, FIELD_WIDTH, FLOAT_PRECISION, bw);
                }
            }
        }
    }
